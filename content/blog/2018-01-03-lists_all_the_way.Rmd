...
If you look at the source code of `map_df()` you see that `dplyr::bind_rows` gets called in the end:

```{r}
map_df
```

So moraly of the story? There are a lot of variants of the common `purrr::map()` function (as well
as of `dplyr` verbs, such as `filter_at`, `select_if`, etc...) and learning about them can save you
from a lot of pain! However, if you need to apply function to nested lists this is still possible;
you just have to think about the structure of the nested list a bit. There is also another function
