...
<img src="/img/santa_tux.jpg" width="100%"/>
:::

*This is part 8 of a series of blog posts about Nix. Check out
the other parts [here](https://www.brodrigues.co/tags/nix/).
...
I wanted to quickly discuss about `nixpkgs`, which is the collection of packages
that can be installed using Nix. Why is a project like Nix and `nixpkgs`
...
`nixpkgs` is "just" a Github repository containing thousands upon thousands of
Nix expressions. When installing a package, these expressions get evaluated, and
the package in question gets installed. What *installed* means can vary:
...
Github repository, and then *installs* it. The installation process in this case
...
[here](https://github.com/NixOS/nixpkgs/pull/263108) is a draft PR I opened to
prepare for the imminent release of Quarto `1.4`. My goal when I opened this
draft PR was to get used to contributing to `nixpkgs` (this was my second or
third PR to `nixpkgs`, and I did some rookie mistakes when opening my first
...
*exotic* systems) (Note: in the case of Quarto, I think the Quarto devs
...
[Quarto’s issue tracker](https://github.com/quarto-dev/quarto-cli/issues/7344)
...
on Nix, they write the expression and open a PR to `nixpkgs`. But this costs
money: so I started a Gofundme which you can find
[here](https://www.gofundme.com/f/package-rstudio-for-nix-on-macos-platforms)
...
<p>Hope you enjoyed! If you found this blog post useful, you might want to
...
```
<p><link href="https://fonts.googleapis.com/css?family=Cookie" rel="stylesheet"><a class="bmc-button" target="_blank" href="https://www.buymeacoffee.com/brodriguesco"><img src="https://www.buymeacoffee.com/assets/img/BMC-btn-logo.svg" alt="Buy me an Espresso"/>[Buy
me an Espresso]{style="margin-left:5px"}</a></p>