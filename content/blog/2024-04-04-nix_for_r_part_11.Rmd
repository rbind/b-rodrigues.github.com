---
date: 2024-04-04
title: Reproducible data science with Nix, part 10 -- build and cache binaries with Github Actions and Cachix
tags: [R, Nix]
menu:
main:
  parent: Blog
...
Cachix is an amazing tool that makes it incredibly easy to set up your own cache. Simply build the packages once,
...
So now, here is what I do with my packages: I define a `default.nix` file that defines an environment
...
is this file that defines the environment for my `{rix}` package. As you can see on line 2, the `rstats-on-nix`
...
then, I simply define my environment. 

Then, still on `{rix}`’s repository, I defined a new action that builds this environment periodically, but
...
get rebuilt nor cached in mine; they simply continue getting pulled. This makes using the development environment very easy, and
